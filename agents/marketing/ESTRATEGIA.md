# Estrategia de Marketing - PyroGuard Chile

## Objetivo Principal
Posicionar a PyroGuard como la solución líder en detección de incendios forestales con drones en Chile, generando leads cualificados para el equipo de ventas.

## Público Objetivo

### Primario (Decision Makers)
- Gerentes de operaciones de empresas forestales
- Directores de CONAF y organismos públicos
- Alcaldes y encargados de emergencia municipal
- Gerentes de riesgo en agroindustria

### Secundario (Influenciadores)
- Bomberos y equipos de emergencia
- Ingenieros forestales
- Académicos y centros de investigación
- Periodistas de medios ambientales

## Pilares de Contenido

### 1. Educativo (40%)
- Cómo funcionan los drones térmicos
- Estadísticas de incendios en Chile
- Tecnología DJI Mavic 3T explicada
- Casos de éxito internacionales

### 2. Producto/Servicio (30%)
- Demos de detección térmica
- Specs técnicos
- Proceso de implementación
- Antes/después con PyroGuard

### 3. Credibilidad (20%)
- Partner Dronivo (Alemania)
- Certificaciones y tecnología
- Testimonios y casos
- Cobertura de prensa

### 4. Engagement (10%)
- Noticias de incendios actuales
- Tips de prevención
- Interacción con la comunidad
- Behind the scenes

## Frecuencia de Publicación

| Red Social | Frecuencia | Mejor Horario |
|------------|------------|---------------|
| LinkedIn | 3x semana | Mar-Jue 9-11am |
| Instagram | 5x semana | Lun-Vie 12-2pm |
| Twitter/X | 1x día | 8am, 1pm, 6pm |
| Facebook | 3x semana | Mié-Vie 3-5pm |

## KPIs Mensuales

### Awareness
- Alcance total: >50,000
- Impresiones: >100,000
- Nuevos seguidores: >500

### Engagement
- Tasa de engagement: >3%
- Comentarios: >100
- Compartidos: >50

### Conversión
- Clicks a web: >500
- Formularios: >20
- Reuniones agendadas: >5

## Hashtags Principales

### Siempre usar:
- #PyroGuard
- #DetecciónDeIncendios
- #DronesChile

### Rotativos:
- #IncendiosForestales
- #TecnologíaTérmica
- #ProtecciónForestal
- #PrevenciónIncendios
- #DJIMavic3T
- #MonitoreoAéreo
- #ChileProtegido

## Tono de Comunicación

### LinkedIn: Profesional y técnico
"Nuestro sistema detecta anomalías térmicas de 0.1°C, permitiendo identificar focos de incendio hasta 2 horas antes de que sean visibles."

### Instagram: Visual y emotivo
"Cada minuto cuenta. Con PyroGuard, detectamos en menos de 5. 🔥🚁"

### Twitter: Directo y noticioso
"ALERTA: 5,500 hectáreas quemadas en 2 horas. Con detección temprana, esto se puede evitar. #PyroGuard"

### Facebook: Cercano y comunitario
"Proteger los bosques de Chile es tarea de todos. Nosotros ponemos la tecnología, ¿nos ayudas a difundir? 🌲"

## Calendario Mensual Tipo

### Semana 1: Educativo
- Lun: Stat de incendios (Tw/Li)
- Mié: Cómo funciona detección térmica (Ig/Fb)
- Vie: Tecnología DJI explicada (Li)

### Semana 2: Producto
- Lun: Demo de detección (Ig/Tw)
- Mié: Specs técnicos (Li)
- Vie: Proceso de implementación (Fb)

### Semana 3: Credibilidad
- Lun: Partner Dronivo (Li/Ig)
- Mié: Caso de éxito (Tw/Fb)
- Vie: Certificaciones (Li)

### Semana 4: Engagement
- Lun: Noticia actual (Tw)
- Mié: Behind the scenes (Ig)
- Vie: CTA contacto (Todas)
